...
class SearchForm(forms.Form):
    ...
    length = forms.TypedChoiceField(choices = LENGTH_CHOICES, coerce=int, empty_value=0, required=False, label='Preferred course duration:')
...
class RegisterForm(UserCreationForm):
    class Meta:
        model = Student
        fields = ['first_name', 'last_name', 'username', 'email', 'level', 'address', 'province', 'registered_courses', 'interested_in']
